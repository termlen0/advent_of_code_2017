...
def clean_input(input_data):
    ...
    for line in input_data.splitlines():
        ...
        result[parent] = children.strip()
    return result


# Collapse top level children
def sort_progs(input_data):
    result = {}
    for parent, children in input_data.items():
        if not children:
            # Top level programs, find their parents
            result[parent] = get_parent(parent, input_data)
        else:
            for child in children.split(','):
                child = child.strip()
                result[child] = get_parent(child, input_data)
    return result


# Helper for sort_progs
def get_parent(child, data):
...
def build_hierarchies(input_data):
    final = {}
    for parent in input_data.values():
        final[parent] = {}
