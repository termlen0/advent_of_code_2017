...
def valid_phrases(input_data):
    """PART 1"""
    valid_phrase_count = 0
    for phrase in input_data.splitlines():
        if len(phrase.split()) == len(set(phrase.split())):
            valid_phrase_count += 1
    return(valid_phrase_count)


def check_anagrams(input_data):
    """PART 2"""
    valid_phrase_count = 0
    for phrase in input_data.splitlines():
        sorted_phrase = [''.join(sorted(word)) for word in phrase.split()]
        if len(set(sorted_phrase)) == len(phrase.split()):
            valid_phrase_count += 1
    return valid_phrase_count


if __name__ == '__main__':
    input_data = """vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
"""
    print(valid_phrases(input_data))  # Expected 451 - PART 1
    print(check_anagrams(input_data))  # Expected 223 - PART 2
