...
def update_register(structured_instructions):
    math_map = dict(inc=1, dec=-1)
    reg_map = {inst[0]: 0 for inst in structured_instructions}
    max_val = 0
    for inst in structured_instructions:
        condition = False
        cond = inst[-3:]
        cond_reg_val = reg_map.get(cond[0])
        cond_symb = cond[1]
        cond_val = int(cond[2])
        if cond_symb == '==':
            if cond_reg_val == cond_val:
                condition = True
        elif cond_symb == '!=':
            if cond_reg_val != cond_val:
                condition = True
        ...
        if condition:
            register = inst[0]
            operand = math_map[inst[1]]
            value = int(inst[2]) * operand
            reg_map[register] += value
            if reg_map[register] > max_val:
                max_val = reg_map[register]
    return reg_map, max_val


def get_structured_input(input_data):
    inst_set = []
...
def get_max(input_data):
    structured_data = get_structured_input(input_data)
    updated_register, max_val = update_register(structured_data)
    return max(updated_register.values()), max_val
